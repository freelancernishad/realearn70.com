<template>
    <section id="bigbg">
        ...
        <h2 class="text-center mb-5">REAL EARN</h2>
...
this.form.country = '+880'
    ...
    methods: {
        ...
        async checkBrowser(){
            ...
            var visitorIds = visitorData.visitorId;
            ...
            if(localStorage.getItem('dmdevice')){
                ...
                var res = await this.callApi('get',`/api/count/username/check?visitorId=${visitorIds}`,[]);
                if(res.data){
                    this.$router.push({ name: '/login' })
                Notification.customError(`This device has already have an account!`);
                localStorage.setItem('dmdevice',1)
